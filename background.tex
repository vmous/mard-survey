...
Early overlay protocol implementations gave litte attention to the 
topology mismatch problem.
{\sl Gnutella}, for example, is
...
which needs global knowledge of the overlay topology, imposes on the system is
greater compared to a protocol that only relies on peers exchanging routing
tables within a 2-hop radius from each other.
