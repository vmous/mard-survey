...
\textbf{\emph{Global Soft-State}} \cite{XTZ2003} builds a global view to help nodes
...
\textbf{\emph{Mithos}}~\cite{WR2003} ensures that neighboring nodes in the overlay are
...
discovered during the aforementioned phase. % iterative selection phase.
...
\textbf{\emph{LAPTOP}}~\cite{WLH2007} organizes the overlay 
...
\textbf{\cite{KLKP2008}} introduces proximity
...
\emph{MaxMind GeoIP} database\footnote{\url{http://www.maxmind.com}} 
...
\textbf{\emph{CHOP6}} \cite{MT2007} is a Chord-variant DHT that exploits
\emph{IPv6} address format as well as available \emph{RTT} information to 
...
\textbf{\emph{T2MC}}'s~\cite{SLCGZ2008} objective is to diminish the redundant multiple
...
The algorithm uses \emph{traceroute}-logs to detect inter-\emph{AS} boundaries 
...
minimum and maximum latencies in the above \emph{traceroute} path 
...
disable \emph{traceroute} support on their routers and this 
undoubtedly affects \emph{T2MC}'s operation.
%%
...
\textbf{\emph{PChord}} \cite{HLYW2005} is based on the \emph{Chord DHT} and incorporates
...
\textbf{\emph{AChord}} \cite{DK2006} exploits IPv6's \emph{anycast} mechanism to
...
\textbf{\emph{Chord6}}~\cite{XZHL2005} is another \emph{Chord}-variant that
...
the \emph{Chord DHT} is introduced by \cite{DLTZZ2006} (\textbf{\emph{DHT-PNS}}).
...
%%
\emph{DHT-PNS} fares as follows in reference to the stated three criteria:
\begin{center}
{\footnotesize
\begin{tabular}{rccc}
\multicolumn{1}{r}{} &
...
\cite{SZ2008} propose \textbf{\emph{Quasi-Chord}}, a \emph{Chord}-based network that
...
\textbf{\emph{IP-based clustering}} or \textbf{\emph{IPBC}}~\cite{KM2007} uses IP address prefixes
...
in the DHT. 
Newly joining nodes with the same \emph{IP} prefix 
...
\textbf{\emph{DynaMO}}~\cite{WZS2004} creates an overlay that 
...
\textbf{\emph{Random LandMarking (RLM)}} and 
\textbf{\emph{Closest Neighbor Prefix Assignment (CNPA)}}.
...
The new-comer takes the bootstrapping node's \emph{ID} prefix 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\textbf{\emph{3D Overlay (3DO)}}~\cite{AOS2014} seeks to address the 
...
\textbf{\emph{SAT-Match}}~\cite{RGJZ2004} defines an iterative process that 
...
\end{enumerate}
%%
Trends for structured systems depicted in Figure~\ref{structured:timeline}
show the same lack of paradigm shift we identified for the unstructured
approaches (check Figure~\ref{unstructured:timeline}). Leveraging proximity
information for selecting the next hop or to construct and maintain
routing tables as a whole, are the two predominant elements used 
throughout the years in 
the structured systems literature.
%%%
...
compared with the trends in unstructured systems is the fact that, here, researchers
seem to juggle with more methods at the same time. This, and the
fact that these suggested solutions are also heuristic in nature, opens them up 
to trade-offs that are more difficult to manage and are highly 
affected by both the application domain and the implementation itself.
...
Table~\ref{structured:table} outlines key properties for all surveyed approaches 
...
Figures~\ref{structured:plot:efficiency}, \ref{structured:plot:overhead}
...
criteria we defined in subsection~\ref{background:motivation}. Finally,
we embed algorithms and characteristics on the temporal axis in
Figure~\ref{structured:timeline} to help depict emerging trends during
