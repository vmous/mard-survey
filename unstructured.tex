...
% process
low &
% the process in not started from the begining at each iteration step
low &
% 
medium \\
\emph{Directed BFS} &
...
% spirral).
medium &
% BFS is Gnutella's non efficient communcation method
medium \\
\emph{Local Indices} &
...
% topology mismatch and thus DCMP minimises this as possible.
...
% query ultimatelly got fulfilled
medium \\
\end{tabular}
\end{center}
...
%
medium &
% tracking of communication costs to all peer's neighbours as well as between
% any pair of neighbours. Whenever a new neighbour cost table is received or
% there is a change of neighbours, the source peer has to re-calculate the
...
high &
% compared to LTM it has slower convergence speed thus incurres more, total,
% overhead.
medium &
%
medium
...
% 55 percent.
low &
% no clock synching needed, fully distributed and with low control overhead the
...
low &
% the selection of neighbours is done at join time. No overlay adaptation means
...
\emph{Efficiency} & \emph{Overhead} & \emph{Scalability} \\
\hline
% peer neighbourhood selected to minimise, for example, average delay
high &
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Biased Neighbour Selection (BNS)}
...
\emph{Efficiency} & \emph{Overhead} & \emph{Scalability} \\
\hline
% inter-ISP cost minimised while retaining the near optimal performance of
% BitTorrent download.
high &
...
\emph{Efficiency} & \emph{Overhead} & \emph{Scalability} \\
\hline
% Authors claim minimisation of inter-ISP traffic cost while enhancing the,
...
\emph{Efficiency} & \emph{Overhead} & \emph{Scalability} \\
\hline
% minimisation of inter-AS traffic but unfair among peers.
medium &
% info (AS hop distances) aqcuired at swarm join
low &
%
high
\end{tabular}
...
% rathier coarse grained since it is taken into account on the AS level.
medium &
% the algoritm does not involve extensive calculations.
low &
...
medium &
% traceroute and hop counts and calculation from both the tracker and the peers
...
\emph{Efficiency} & \emph{Overhead} & \emph{Scalability} \\
\hline
% It incoorporates clustering of peers to inter- and intra ISP which is a
...
% being close or distant the actual computation is done in a distributed mannar
% thus not incurring too much additional controlling overhead.
medium &
% TODO: how intensive with regards to computing power is an online k-means (with
% k=3) classification???
medium
\end{tabular}
\end{center}
...
% better performance
medium &
% Exchanging neighbours within a radius of 2 (TTL value) reduces stretch
...
medium \\
\emph{PROP-O} &
% The algorithm can better reclaim the heterogenity of peers (those with faster
...
% established.
low &
% it can be utilised in hierarchical architectures as well meaning increased
...
% decentralised, thus preventing it from scalling smoothly. Communicating
...
% decentralised, thus preventing it from scalling smoothly. Communicating
% and overloading landmark servers in high-churn systems is another scalability
% concern.ctured architectures and we visited a series
...
Table~\ref{unstructured:table} offers a summarised overview of the algorithms
...
\begin{figure}[ht]
\centering
\subfigure[Ineffictient overlay topology.] {
...
Concluding, topology adaptation ultimatelly targets on the reduction of the
...
\includegraphics[scale=0.25]{img/pdf/forwarding-optimisation-before.pdf}
  \label{figure:forwarding-optimisation:before}
}\qquad\qquad
\subfigure[A node can decide where to forward the messages.] {
  \includegraphics[scale=0.25]{img/pdf/forwarding-optimisation-after.pdf}
  \label{figure:forwarding-optimisation:after}
}
\caption{Forwarding optimisation in constraining message flooding.}
...
% as well as content distribution networks. Application layer multicast, however,
...
\subfigure[Data replication allong the path of a successful query.] {
